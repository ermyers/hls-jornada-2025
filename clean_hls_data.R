...
return_eco_state <- function(phenocam_name){
  if(phenocam_name == "jershrubland"){
    ...
    eco_state = "Sandy grassy shrubland"
...
hls_csv_to_df <- function(filename){
  ...
  processed_data <- data.frame(DATE = calc_date(hls_data$Year, hls_data$DOY),
                               ...
                               BLUE = hls_data$Blue_mean,
                               GREEN = hls_data$Green_mean,
                               RED = hls_data$Red_mean,
                               NIR = hls_data$NIRNarrow_mean,
...
# Save results
save(hls_center_clean, file="outputs/hls_v20_ec_towers_clean.RData")